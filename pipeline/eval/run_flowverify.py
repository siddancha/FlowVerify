'''
script to generate FlowVerify scores
'''
...
import flowmatch.test as flow_test
...
from flowmatch.flowscorer import FlowMatchScorer

def split_chunk(l, chunk_size=8):
...
def run_evaluation(dataset, net, args):
    ...
    score_names = FlowMatchScorer.score_names

    def get_scores(examples):
        batches = split_chunk(examples)
        flow_preds_list = []
        for batch in batches:
            flow_preds = flow_test.raw_net_output_batch(net, batch)
            flow_preds = flow_test.postprocess_raw_net_output_batch(flow_preds)
            flow_preds_list += flow_preds

        list_list_score = [[] for _ in score_names]  # each element corresponds to a score type
        for example, flow_pred in zip(examples, flow_preds_list):
            example['pred_flow'] = flow_pred

            # Reclassification.
            reclass_scorer = FlowMatchScorer(example)
            scores = reclass_scorer.get_scores()
            for i, score in enumerate(scores):
                list_list_score[i].append(score)
        return list_list_score
    ...
    for index_ind, index in enumerate(range(0, len(dataset.dt_ids))):
        t1 = time.time()
        ...
        fr_box_list = []
        for vp_id, target_example in enumerate(list_target_example):
            example = {**scene_example, **target_example}
            list_example.append(example)

        list_list_score = get_scores(list_example)  # this modifies the example
        ...
        print('{}/{} ({:.1f}%) detections done with time {:.2f} ...'.format(num_done, num_eval, (num_done / num_eval) * 100., time.time()-t1))

    evaluation_time = time.time() - start
    print('Total evaluation time: {:.2f}s'.format(evaluation_time))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='flow', help='flow/sift')
